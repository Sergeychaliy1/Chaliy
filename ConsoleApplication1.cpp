...
void SetColor(int text, int background)  // ФУНКЦИЯ, МЕНЯЮЩАЯ ЦВЕТ ТЕКСТА И ЦВЕТ ФОНА В КОНСОЛИ.
{
...
void CursoreCord(int x, int y)  //ФУНКЦИЯ,МЕНЯЮЩАЯ ПОЛОЖЕНИЯ КУРСОРА НА КООРДИНАТЫ (X,Y)
{
	HANDLE hCon;
	COORD cPos;
...
{                                  
	switch (_getch())// курсор принимает значение
	{
	case 'w': 
		if (cursore % 5 > 0)      //КУРСОР СМЕЩАЕТСЯ ВВЕРХ,  ЕСЛИ НЕ НАХОДИТСЯ В ВЕРХНЕЙ ТОЧКЕ
			return cursore - 1;
		else
			return cursore;
		break;
	...
	case 72://КЛАВИША СТРЕЛОЧКА ВВЕРХ
		...
		break;
	case 's': 
		if (cursore % 5 < 4)      //КУРСОР СМЕЩАЕТСЯ ВНИЗ,  ЕСЛИ НЕ НАХОДИТСЯ В НИЖНЕЙ ТОЧКЕ
			return cursore + 1;
		else
			return cursore;
		break;
	...
	case 13:                      //КЛАВИША ENTER
		if (cursore < 5) 
		{
			if (cursore == 4) //ЕСЛИ Enter СРАБОТАЛ НА КНОПКЕ 4 (ВЫХОД) ТО ЗАМЕНЯЕМ MENU НА FALSE ,ПРОИСХОДИТ ВЫХЛД ИЗ МЕНЮ
			{
				menu = false;
				return 0;
			}
		...
		else if (cursore % 5 == 4) //ЕСЛИ КУРСОР СТОИТ НА КНОПКЕ "НАЗАД  " ТО КУРСОР СМЕЩАЕТСЯ В ГЛАВНОЕ МЕНЮ
		{
			system("cls");
...
int main()
{
	...
	while (bool menu = TRUE) // ПОКА MENU == TRUE
	{
		...
		cursore = Key(cursore, menu); //КУРСОР ИЗМЕНИЕТСЯ ЕСЛИ НАЖАТЫ СТРЕЛОЧКИ ИЛИ ENTER
	}
}